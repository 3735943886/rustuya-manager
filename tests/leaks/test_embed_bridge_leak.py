...
`start_async()` blocks on an `asyncio.Event` until `stop()` trips it — that
...
def _make_blocking_server(**_kw):
    """Stub PyBridgeServer whose start_async() blocks until stop() is called.
    ...
    s.start_async.side_effect = _start_async
...
async def test_embed_resolve_cycle_no_leak(tmp_path: Path):
    ...
    async def one_cycle():
        with patch("pyrustuyabridge.PyBridgeServer", side_effect=_make_blocking_server):
            supervisor, task = _spawn_embedded_bridge(args)
